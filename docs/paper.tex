...
2D-ROCKET        & NEU         & \textbf{0.9556} & \textbf{0.9555} & 319.7 \\
\midrule
Lightweight CNN & Chest X-Ray & \textbf{0.9514} & \textbf{0.9373} & 76.8 \\
...
\addplot coordinates {(NEU,89.72) (Chest X-Ray,95.14) (DTD,11.52)};
...
\addplot coordinates {(NEU,18.7)[19] (Chest X-Ray,76.8)[77] (DTD,59.2)[59]};
...
Results are summarized in Table~\ref{tbl:realworld} and Fig.~\ref{fig:realworld-acc}--\ref{fig:cm-xray}. On NEU, a genuinely texture-dominated industrial inspection task, 2D-ROCKET clearly outperforms the CNN (95.56\% vs.\ 89.72\%), reproducing the pattern already observed on KTH-TIPS. Fig.~\ref{fig:cm-neu} shows why: the CNN's errors concentrate heavily in one confusable pair (15 of 60 \emph{scratches} images predicted as \emph{inclusion}), whereas 2D-ROCKET's errors are smaller in number and more evenly spread across classes, with every per-class recall at 92\% or above. On DTD, a substantially harder 47-class fine-grained texture benchmark, both lightweight models struggle in absolute terms with no pretraining or augmentation at $128\times128$ resolution, but the relative gap remains large and in 2D-ROCKET's favor (28.28\% vs.\ 11.52\%, more than double), consistent with the hypothesis that a flat bank of random, multi-scale local-statistic features is better matched to texture-attribute discrimination than a two-layer CNN with no capacity for hierarchical composition; at 47 classes the full confusion matrix is not legible at print size and is omitted here, but is included among the run artifacts released with this work. Chest X-Ray Pneumonia inverts this pattern: the CNN reaches 95.14\% against 2D-ROCKET's 92.83\%. Pneumonia is diagnosed less from repetitive local texture than from spatially localized opacity patterns -- effectively a coarse shape/structure cue overlaid on lung anatomy -- and even a two-layer CNN's learned, spatially adaptive filters appear better matched to isolating this signal than a fixed random-kernel bank tuned for statistical, non-compositional texture cues. Fig.~\ref{fig:cm-xray} shows this gap is concentrated in missed pneumonia cases specifically, the clinically costlier error: the CNN misses 20 of 855 pneumonia cases (2.3\%) against 39 of 855 (4.6\%) for 2D-ROCKET, roughly double the false-negative rate. This inversion is itself informative rather than contradictory: it is the one dataset in this extended evaluation that is not primarily texture-dominated, and it is exactly there that the CNN regains its edge, sharpening rather than undermining the domain boundary this paper argues for. Across all three datasets, 2D-ROCKET's total (uncached) run time remains substantially higher than the CNN's -- roughly 5--17$\times$ depending on dataset size -- since, unlike the cached-feature timings in Table~\ref{tbl:main}, feature-extraction cost is included here. As with the rest of this study, these results reflect a single seed and no dataset-specific retuning (the CIFAR-10-tuned configuration was carried over unmodified, as for KTH-TIPS), and should be read as an initial, real-world-oriented validation rather than an exhaustive benchmark.

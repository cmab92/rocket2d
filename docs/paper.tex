...
\addplot coordinates {(NEU,96.67) (Chest X-Ray,95.56) (DTD,8.51)};
\addplot coordinates {(NEU,95.56) (Chest X-Ray,92.83) (DTD,28.28)};
\legend{Lightweight CNN, 2D-ROCKET}
\end{axis}
\end{tikzpicture}
\caption{Test accuracy (\%) of the lightweight CNN baseline and the final 2D-ROCKET configuration on NEU, Chest X-Ray Pneumonia, and DTD.}\label{fig:realworld-acc}
\end{figure}
...
\caption{NEU confusion matrices (60 test images per class).}\label{fig:cm-neu}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figures/cm_xray.png}
\caption{Chest X-Ray Pneumonia confusion matrices (317 NORMAL / 855 PNEUMONIA test images).}\label{fig:cm-xray}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figures/dtd_cnn_learning_curve.png}
\caption{DTD lightweight-CNN training loss over the full 30-epoch schedule. Loss falls below 0.2 with no plateau, while held-out test accuracy \emph{worsens} relative to a 10-epoch run (11.52\%$\to$8.51\%) -- a classic overfitting signature on a dataset with only $\sim$96 training images per class and no augmentation.}\label{fig:dtd-overfit}
\end{figure}

Results are summarized in Table~\ref{tbl:realworld} and Fig.~\ref{fig:realworld-acc}--\ref{fig:dtd-overfit}. On NEU, the CNN reaches 96.67\% against 2D-ROCKET's 95.56\% (Fig.~\ref{fig:cm-neu}): both models now make few, evenly spread errors (at most 2 per off-diagonal cell for the CNN, at most 5 for 2D-ROCKET), and the CNN's small edge runs counter to this paper's original expectation that a texture-dominated task like NEU should favor 2D-ROCKET, as it did on KTH-TIPS. On Chest X-Ray Pneumonia the CNN's advantage is larger, 95.56\% against 92.83\%; Fig.~\ref{fig:cm-xray} shows this concentrated in missed pneumonia cases, the clinically costlier error type: the CNN misses 23 of 855 cases (2.7\%) against 2D-ROCKET's 39 (4.6\%).

DTD tells a different, more diagnostic story. An initial 10-epoch CNN schedule reached 11.52\% on DTD, but its training loss was still falling steeply; extending training to 30 epochs -- the schedule ultimately used uniformly across all three datasets, and the one reported in Table~\ref{tbl:realworld} -- drove the CNN's training loss below 0.2 (Fig.~\ref{fig:dtd-overfit}), essentially memorizing the roughly 96 training images available per class with no augmentation, while held-out accuracy \emph{fell} to 8.51\%. 2D-ROCKET's 28.28\% on the same split is unaffected by this failure mode by construction: its only trained component is a linear Ridge classifier over 25,000 fixed random features, with no epoch budget or convergence check involved. Rather than confirming that 2D-ROCKET specifically suits texture-dominated data -- the NEU result above already complicates that reading -- DTD is better explained by 2D-ROCKET's implicit regularization: frozen random features plus a single linear layer are a far lower-capacity fit than a CNN with learned convolutional weights, making it substantially more sample-efficient and overfitting-resistant in a genuinely low-data, per-class regime, independent of whether the underlying data is texture-like.

Across all three datasets, 2D-ROCKET's total (uncached) run time remains 5.4--6.4$\times$ higher than the CNN's, since, unlike the cached-feature timings in Table~\ref{tbl:main}, feature-extraction cost is included here. As with the rest of this study, these results reflect a single seed and no dataset-specific retuning beyond the uniform epoch-count correction described above, and should be read as an initial, real-world-oriented validation rather than an exhaustive benchmark.
...
The extended, real-world evaluation on NEU, Chest X-Ray Pneumonia, and DTD (Sec.~\ref{sec:realworld}) complicates this paper's texture-domain narrative more than it confirms it, which is itself a useful result. Once the CNN baseline is trained to convergence, it edges out 2D-ROCKET on both NEU and Chest X-Ray -- including NEU, a genuinely texture-dominated industrial task that, by the logic developed from KTH-TIPS, should have favored 2D-ROCKET. Only DTD still favors 2D-ROCKET decisively (28.28\% vs.\ 8.51\%), and the more defensible explanation is not texture-domain fit but sample efficiency: DTD offers only about 96 training images per class with no augmentation, a regime in which the from-scratch CNN overfits sharply (Fig.~\ref{fig:dtd-overfit}) while 2D-ROCKET's fixed-feature, linear-classifier design cannot. The KTH-TIPS result and the DTD result are therefore both consistent with 2D-ROCKET being favored in low-data, low-augmentation regimes, but the mechanism looks more like implicit regularization from a low-capacity linear fit over frozen features than a texture-specific inductive bias. This is a narrower and more defensible claim than the one this paper's ablations alone would suggest, and it is the kind of correction that only an out-of-sample, real-world evaluation can surface.
...
2D-ROCKET is best understood as a lightweight, reproducible, and interpretable baseline for low-data, low-augmentation regimes, not as a replacement for deep networks on general vision tasks that require multi-scale object composition -- and, per Sec.~\ref{sec:realworld}, not straightforwardly as a texture-domain specialist either. An initial step toward the systematic, real-world evaluation this scope calls for was carried out in Sec.~\ref{sec:realworld}: the unmodified, CIFAR-10-tuned 2D-ROCKET configuration was applied to industrial surface-defect (NEU), texture-attribute (DTD), and medical (Chest X-Ray Pneumonia) imagery. A properly converged CNN baseline outperformed 2D-ROCKET on two of the three datasets, including the texture-dominated NEU task, while 2D-ROCKET's clear advantage on DTD tracked that dataset's small per-class sample size rather than its texture content -- pointing to sample efficiency under limited, unaugmented data as a better-supported explanation for 2D-ROCKET's real-world niche than texture-domain specificity alone. Remaining promising directions for future work include multi-seed statistical validation, domain-aware or structured (rather than purely random) kernel sampling, hybrid pipelines that combine random convolutions with a small trainable component to recover some hierarchical composition, dimensionality reduction to control feature-storage cost at large kernel budgets, and a systematic study that varies training-set size and augmentation directly to test the sample-efficiency hypothesis raised by Sec.~\ref{sec:realworld}.

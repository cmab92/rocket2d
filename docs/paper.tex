...
Code for the real-world validation study (Sec.~\ref{sec:realworld}), the LSPV feature (Sec.~\ref{sec:lspv}), and its capacity-control experiment, including the exact scripts used to produce Tables~\ref{tbl:realworld}--\ref{tbl:lspv} and Figs.~\ref{fig:realworld-acc}--\ref{fig:dtd-overfit}, is available in the accompanying repository (see \texttt{docs/REPRODUCING.md} for step-by-step instructions). NEU, Chest X-Ray Pneumonia, and DTD are third-party datasets accessed via their public Kaggle mirrors and are not redistributed. The MNIST/CIFAR-10/KTH-TIPS ablation study underlying Table~\ref{tbl:main} and Sec.~\ref{sec:results} up to Sec.~\ref{sec:realworld} was conducted separately from this repository and is not reproducible from the code released alongside this paper; it is documented in the corresponding bachelor thesis.
